...
class AbuseIPDBServer:
    ...
    async def check_ip(self, args: Dict[str, Any]) -> CallToolResult:
        ...
        if not self.api_key:
            return CallToolResult(
                content=[TextContent(type="text", text="❌ ABUSEIPDB_API_KEY environment variable is required")],
                isError=True,
            )

        if not self.is_valid_ip(ip_address):
            return CallToolResult(
                content=[TextContent(type="text", text="❌ Invalid IP address format")],
                isError=True,
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                return CallToolResult(content=[TextContent(type="text", text=self.format_check_response(data))])

        except Exception as error:
            return CallToolResult(
                content=[TextContent(type="text", text=f"❌ API request failed: {str(error)}")],
                isError=True,
            )
    ...
    async def report_ip(self, args: Dict[str, Any]) -> CallToolResult:
        ...
        if not self.is_valid_categories(categories):
            return CallToolResult(
                content=[TextContent(type="text", text="❌ Categories must be comma-separated integers")],
                isError=True,
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                return CallToolResult(content=[TextContent(type="text", text=self.format_report_response(data))])
    ...
    def handle_api_error(self, response: httpx.Response, data: Dict[str, Any]) -> CallToolResult:
        ...
        if status == 429:
            ...
            return CallToolResult(
                content=[TextContent(type="text", text=f"❌ {error_message}\n\nError details: {data}")],
                isError=True,
            )
        ...
        return CallToolResult(
            content=[TextContent(type="text", text=f"❌ API Error ({status}): {error_detail}\n\nFull response: {data}")],
            isError=True,
        )
